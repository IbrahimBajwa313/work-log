import type { AdhkarItem } from "@/lib/azkar";

export const EVENING_ADHKAR_INTRO =
  "The evening adhkār are remembrances and duʿās prescribed by our beloved Prophet ﷺ. Recite them with understanding and presence of heart. The best time is between ʿAsr and Maghrib; if you miss that, make them up later. If you cannot recite them all, be consistent with at least some.";

export const EVENING_ADHKAR: AdhkarItem[] = [
  {
    id: "ayat-al-kursi",
    title: "Ayat al-Kursi: The Greatest Protection",
    arabic:
      "أَعُوْذُ بِاللّٰهِ مِنَ الشَّيْطَانِ الرَّجِيْمِ. اَللّٰهُ لَآ إِلٰهَ إِلَّا هُوَ الْحَىُّ الْقَيُّوْمُ ، لَا تَأْخُذُهُۥ سِنَةٌ وَّلَا نَوْمٌ ، لَهُ مَا فِى السَّمٰـوٰتِ وَمَا فِى الْأَرْضِ ، مَنْ ذَا الَّذِىْ يَشْفَعُ عِنْدَهُ إِلَّا بِإِذْنِهِۦ ، يَعْلَمُ مَا بَيْنَ أَيْدِيْهِمْ وَمَا خَلْفَهُمْ ، وَلَا يُحِيْطُوْنَ بِشَىْءٍ مِّنْ عِلْمِهِٓ إِلَّا بِمَا شَآءَ ، وَسِعَ كُرْسِيُّهُ السَّمٰـوٰتِ وَالْأَرْضَ، وَلَا يَئُوْدُهُۥ حِفْظُهُمَا ، وَهُوَ الْعَلِىُّ الْعَظِيْمُ.",
    repeatCount: 1,
    translation:
      "I seek the protection of Allah from the accursed Shayṭān. Allah, there is no god worthy of worship but He, the Ever Living, The Sustainer of all. Neither drowsiness overtakes Him nor sleep. To Him Alone belongs whatever is in the heavens and whatever is on the earth. Who is it that can intercede with Him except with His permission? He knows what is before them and what will be after them, and they encompass not a thing of His knowledge except for what He wills. His Kursī extends over the heavens and the earth, and their preservation does not tire Him. And He is the Most High, the Magnificent. (2:255)",
    virtue:
      "Ubayy b. Kaʿb (raḍiy Allāhu ʿanhu) reported that a jinnī told him: \"If you read Āyah al-Kursī in the morning, you will be protected from us till the evening. And if you read it in the evening, you will be protected from us till the morning.\" The Messenger of Allah ﷺ said: \"The evil one spoke the truth.\" (Ḥākim 2064)",
  },
  {
    id: "three-quls",
    title: "3 Quls: To Suffice You in All Your Matters",
    arabic:
      "بِسْمِ اللّٰهِ الرَّحْمٰنِ الرَّحِيْمِ. قُلْ هُوَ اللّٰهُ أَحَدٌ ، اَللّٰهُ الصَّمَدُ ، لَمْ يَلِدْ وَلَمْ يُوْلَدْ ، وَلَمْ يَكُنْ لَّهُ كُفُوًا أَحَدٌ.\n\nبِسْمِ اللّٰهِ الرَّحْمٰنِ الرَّحِيْمِ. قُلْ أَعُوْذُ بِرَبِّ الْفَلَقِ ، مِنْ شَرِّ مَا خَلَقَ ، وَمِنْ شَرِّ غَاسِقٍ إِذَا وَقَبَ ، وَمِنْ شَرِّ النَّفَّاثَاتِ فِي الْعُقَدِ ، وَمِنْ شَرِّ حَاسِدٍ إِذَا حَسَدَ.\n\nبِسْمِ اللّٰهِ الرَّحْمٰنِ الرَّحِيْمِ. قُلْ أَعُوْذُ بِرَبِّ النَّاسِ ، مَلِكِ النَّاسِ ، إِلٰهِ النَّاسِ ، مِنْ شَرِّ الْوَسْوَاسِ الْخَنَّاسِ ، اَلَّذِيْ يُوَسْوِسُ فِيْ صُدُوْرِ النَّاسِ ، مِنَ الْجِنَّةِ وَالنَّاسِ.",
    repeatCount: 3,
    translation:
      "In the name of Allah, the All-Merciful, the Very Merciful. Say, He is Allah, the One, the Self-Sufficient Master, Who has not given birth and was not born, and to Whom no one is equal. (112) In the name of Allah, the All-Merciful, the Very Merciful. Say, I seek protection of the Lord of the daybreak, from the evil of what He has created, and from the evil of the darkening night when it settles, and from the evil of the blowers in knots, and from the evil of the envier when he envies. (113) In the name of Allah, the All-Merciful, the Very Merciful. Say, I seek protection of the Lord of mankind, the King of mankind, the God of mankind, from the evil of the whisperer who withdraws, who whispers in the hearts of mankind, whether they be Jinn or people. (114)",
    virtue:
      "The Messenger of Allah ﷺ said: \"Recite Sūrah al-Ikhlās and al-Muʿawwidhatayn three times in the morning and the evening. It will suffice you in all respects.\" 'It will suffice you in all respects' means it will protect you from all evil. (Tirmidhī 3575)",
  },
  {
    id: "sayyid-al-istighfar",
    title: "Sayyid al-Istighfar: The Best Way of Seeking Forgiveness",
    arabic:
      "اَللّٰهُمَّ أَنْتَ رَبِّيْ لَا إِلٰهَ إِلَّا أَنْتَ ، خَلَقْتَنِيْ وَأَنَا عَبْدُكَ ، وَأَنَا عَلَىٰ عَهْدِكَ وَوَعْدِكَ مَا اسْتَطَعْتُ ، أَعُوْذُ بِكَ مِنْ شَرِّ مَا صَنَعْتُ ، أَبُوْءُ لَكَ بِنِعْمَتِكَ عَلَيَّ وَأَبُوْءُ لَكَ بِذَنْبِيْ ، فَاغْفِرْ لِيْ فَإِنَّهُ لَا يَغْفِرُ الذُّنُوْبَ إِلَّا أَنْتَ.",
    repeatCount: 1,
    translation:
      "O Allah, You are my Lord. There is no god worthy of worship except You. You have created me, and I am Your slave, and I am under Your covenant and pledge (to fulfil it) to the best of my ability. I seek Your protection from the evil that I have done. I acknowledge the favours that You have bestowed upon me, and I admit my sins. Forgive me, for none forgives sins but You.",
    virtue:
      "Shaddād b. Aws (raḍiy Allāhu ʿanhu) narrates that the Messenger of Allah ﷺ said: \"Whoever says it during the night with firm belief in it and dies before the morning, he will be from the people of Paradise.\" (Bukhārī 6306)",
  },
  {
    id: "anxiety-laziness-debt",
    title: "Protect Yourself From Anxiety, Laziness, Debt etc",
    arabic:
      "اَللّٰهُمَّ إِنِّيْ أَعُوْذُ بِكَ مِنَ الْهَمِّ وَالْحَزَنِ ، وَأَعُوْذُ بِكَ مِنَ الْعَجْزِ وَالْكَسَلِ، وَأَعُوْذُ بِكَ مِنَ الْجُبْنِ وَالْبُخْلِ ، وَأَعُوْذُ بِكَ مِنْ غَلَبَةِ الدَّيْنِ وَقَهْرِ الرِّجَالِ.",
    repeatCount: 1,
    translation:
      "O Allah, I seek Your protection from anxiety and grief. I seek Your protection from inability and laziness. I seek Your protection from cowardice and miserliness, and I seek Your protection from being overcome by debt and being overpowered by men.",
    virtue:
      "The Messenger of Allah ﷺ taught Abū Umāmah (raḍiy Allāhu ʿanhu) to say this in the morning and evening. Abū Umāmah said: \"Then I did that, and Allah removed my worries and settled my debts.\" (Abū Dāwūd 1555)",
  },
  {
    id: "wellbeing-dunya-akhirah",
    title: "Well-being in this World and the Hereafter",
    arabic:
      "اَللّٰهُمَّ إِنِّيْ أَسْأَلُكَ الْعَافِيَةَ فِي الدُّنْيَا وَالْآخِرَةِ ، اَللّٰهُمَّ إِنِّيْ أَسْأَلُكَ الْعَفْوَ وَالْعَافِيَةَ فِيْ دِيْنِيْ وَدُنْيَايَ وَأَهْلِيْ وَمَالِيْ ، اَللّٰهُمَّ اسْتُرْ عَوْرَاتِيْ وَآمِنْ رَوْعَاتِيْ ، اَللّٰهُمَّ احْفَظْنِيْ مِنْ بَيْنِ يَدَيَّ ، وَمِنْ خَلْفِيْ ، وَعَنْ يَّمِيْنِيْ ، وَعَنْ شِمَالِيْ ، وَمِنْ فَوْقِيْ ، وَأَعُوْذُ بِعَظَمَتِكَ أَنْ أُغْتَالَ مِنْ تَحْتِيْ.",
    repeatCount: 1,
    translation:
      "O Allah, I ask You for well-being in this world and the next. O Allah, I ask You for forgiveness and well-being in my religion, in my worldly affairs, in my family and in my wealth. O Allah, conceal my faults and calm my fears. O Allah, guard me from in front of me and behind me, from my right, and from my left, and from above me. I seek protection in Your Greatness from being unexpectedly destroyed from beneath me.",
    virtue:
      "ʿAbdullāh b. ʿUmar (raḍiy Allāhu ʿanhumā) narrated: \"The Messenger of Allah ﷺ never failed to say these words in the morning and in the evening.\" (Abū Dāwūd 5074)",
  },
  {
    id: "four-evils",
    title: "Protect Yourself From the Four Evils",
    arabic:
      "اَللّٰهُمَّ فَاطِرَ السَّمٰوَاتِ وَالْأَرْضِ ، عَالِمَ الْغَيْبِ وَالشَّهَادَةِ ، رَبَّ كُلِّ شَيْءٍ وَّمَلِيْكَهُ ، أَشْهَدُ أَنْ لَّا إِلٰهَ إِلَّا أَنْتَ ، أَعُوْذُ بِكَ مِنْ شَرِّ نَفْسِيْ ، وَمِنْ شَرِّ الشَّيْطَانِ وَشِرْكِهِ ، وَأَنْ أَقْتَرِفَ عَلَىٰ نَفْسِيْ سُوْءًا أَوْ أَجُرَّهُ إِلَىٰ مُسْلِمٍ.",
    repeatCount: 1,
    translation:
      "O Allah, Creator of the heavens and the earth, Knower of the unseen and the seen, the Lord and Sovereign of everything; I bear witness that there is no god worthy of worship but You. I seek Your protection from the evil of my own self, from the evil of Shayṭān and from the evil of polytheism to which he calls, [and from inflicting evil on myself, or bringing it upon a Muslim].",
    virtue:
      "Abū Bakr al-Ṣiddīq (raḍiy Allāhu ʿanhu) said: \"O Messenger of Allah, instruct me what to say in morning and in the evening.\" He said: \"Say [the above].\" (Composite: Tirmidhī 3392, 3529)",
  },
  {
    id: "entrust-matters",
    title: "Entrust All Your Matters to Allah",
    arabic:
      "يَا حَيُّ يَا قَيُّوْمُ ، بِرَحْمَتِكَ أَسْتَغِيْثُ ، أَصْلِحْ لِيْ شَأْنِيْ كُلَّهُ ، وَلَا تَكِلْنِيْ إِلَىٰ نَفْسِيْ طَرْفَةَ عَيْنٍ.",
    repeatCount: 1,
    translation:
      "O The Ever Living, The Sustainer of all; I seek assistance through Your mercy. Rectify all of my affairs and do not entrust me to myself for the blink of an eye.",
    virtue:
      "Anas b. Mālik (raḍiy Allāhu ʿanhu) narrates that the Prophet ﷺ said to Fāṭimah (raḍiy Allāhu ʿanhā): \"That you say [the above] in the morning and evening?\" (Nasā'ī in ʿAmal al-Yawm wa-l-Laylah 570)",
  },
  {
    id: "thank-allah",
    title: "Fulfil Your Obligation to Thank Allah",
    arabic:
      "اَللّٰهُمَّ مَا أَمْسَىٰ بِيْ مِنْ نِّعْمَةٍ أَوْ بِأَحَدٍ مِّنْ خَلْقِكَ ، فَمِنْكَ وَحْدَكَ لَا شَرِيْكَ لَكَ ، فَلَكَ الْحَمْدُ وَلَكَ الشُّكْرُ.",
    repeatCount: 1,
    translation:
      "O Allah, all the favours that I or anyone from Your creation has received in the evening, are from You Alone. You have no partner. To You Alone belong all praise and all thanks.",
    virtue:
      "ʿAbdullāh b. Ghannām (raḍiy Allāhu ʿanhu) narrates that the Messenger of Allah ﷺ said: \"Whoever says it in the evening has fulfilled his obligation for that night.\" (Abū Dāwūd 5073)",
  },
  {
    id: "renew-tawhid",
    title: "Start Your Evening by Renewing Tawhid",
    arabic:
      "أَمْسَيْنَا عَلَىٰ فِطْرَةِ الْإِسْلَامِ ، وَعَلَىٰ كَلِمَةِ الْإِخْلَاصِ ، وَعَلَىٰ دِيْنِ نَبِيِّنَا مُحَمَّدٍ ، وَعَلَىٰ مِلَّةِ أَبِيْنَا إِبْرَاهِيْمَ حَنِيْفًا مُّسْلِمًا وَّمَا كَانَ مِنَ الْمُشْرِكِيْنَ.",
    repeatCount: 1,
    translation:
      "We have entered the evening upon the natural religion of Islam, the statement of pure faith (i.e. Shahādah), the religion of our Prophet Muhammad ﷺ and upon the way of our father Ibrāhīm, who turned away from all that is false, having surrendered to Allah, and he was not of the polytheists.",
    virtue:
      "ʿAbd al-Raḥmān b. Abzā (raḍiy Allāhu ʿanhu) narrates that the Prophet ﷺ said [the above]. (Aḥmad 15360)",
  },
  {
    id: "praise-allah",
    title: "Start the Evening by Praising Allah",
    arabic: "أَمْسَيْتُ أُثْنِيْ عَلَيْكَ حَمْدًا ، وَأَشْهَدُ أَنْ لَّا إِلٰهَ إِلَّا اللّٰهُ.",
    repeatCount: 3,
    translation:
      "I have entered the evening praising You, and I bear witness that there is no god worthy of worship but Allah.",
    virtue:
      "Abū Hurayrah (raḍiy Allāhu ʿanhu) narrated that the Messenger of Allah ﷺ said: \"When they enter the evening, they should say [the above] three times.\" (Nasā'ī in Sunan al-Kubrā 10406)",
  },
  {
    id: "good-evening",
    title: "Ask Allah for a Good Evening",
    arabic:
      "أَمْسَيْنَا وَأَمْسَى الْمُلْكُ لِلّٰهِ وَالْحَمْدُ لِلّٰهِ ، لَا إِلٰهَ إِلَّا اللّٰهُ وَحْدَهُ لَا شَرِيْكَ لَهُ ، لَهُ الْمُلْكُ وَلَهُ الْحَمْدُ ، وَهُوَ عَلَىٰ كُلِّ شَيْءٍ قَدِيْرٌ ، رَبِّ أَسْأَلُكَ خَيْرَ مَا فِيْ هٰذِهِ اللَّيْلَةِ وَخَيْرَ مَا بَعْدَهَا ، وَأَعُوْذُ بِكَ مِنْ شَرِّ مَا فِيْ هٰذِهِ اللَّيْلَةِ وَشَرِّ مَا بَعْدَهَا ، رَبِّ أَعُوْذُ بِكَ مِنَ الْكَسَلِ وَسُوْءِ الْكِبَرِ ، رَبِّ أَعُوْذُ بِكَ مِنْ عَذَابٍ فِي النَّارِ وَعَذَابٍ فِي الْقَبْرِ.",
    repeatCount: 1,
    translation:
      "We have entered the evening and at this very time the whole kingdom belongs to Allah. All praise is due to Allah. There is no god worthy of worship except Allah, the One; He has no partner with Him. The entire kingdom belongs solely to Him, to Him is all praise due, and He is All-Powerful over everything. My Lord, I ask You for the good that is in this night and the good that follows it, and I seek Your protection from the evil that is in this night and from the evil that follows it. My Lord, I seek Your protection from laziness and the misery of old age. My Lord, I seek Your protection from the torment of the Hell-fire and the punishment of the grave.",
    virtue:
      "ʿAbdullāh b. Masʿūd (raḍiy Allāhu ʿanhu) narrates that when the Messenger of Allah ﷺ used to enter the evening, he would say [the above]. (Muslim 2723)",
  },
  {
    id: "bless-evening",
    title: "Ask Allah to Bless Your Evening",
    arabic:
      "أَمْسَيْنَا وَأَمْسَى الْمُلْكُ لِلّٰهِ رَبِّ الْعَالَمِيْنَ ، اَللّٰهُمَّ إِنِّيْ أَسْأَلُكَ خَيْرَ هـٰذِهِ اللَّيْلَةِ ، فَتْحَهَا وَنَصْرَهَا وَنُوْرَهَا وَبَرَكَتَهَا وَهُدَاهَا ، وَأَعُوْذُ بِكَ مِنْ شَرِّ مَا فِيْهَا وَشَرِّ مَا بَعْدَهَا.",
    repeatCount: 1,
    translation:
      "We have entered the evening and at this very time the whole kingdom belongs to Allah, Lord of the Worlds. O Allah, I ask You for the goodness of this night: its victory, its help, its light, and its blessings and guidance. I seek Your protection from the evil that is in it and from the evil that follows it.",
    virtue:
      "Abū Mālik al-Ashʿarī (raḍiy Allāhu ʿanhu) narrates that the Messenger of Allah ﷺ said: \"When they reach the evening, they should say [the above].\" (Abū Dāwūd 5084)",
  },
  {
    id: "freed-from-hellfire",
    title: "Get Yourself Freed from the Hell-fire",
    arabic:
      "اَللّٰهُمَّ إِنِّيْ أَمْسَيْتُ أُشْهِدُكَ وَأُشْهِدُ حَمَلَةَ عَرْشِكَ وَمَلَائِكَتَكَ وَجَمِيْعَ خَلْقِكَ ، أَنَّكَ أَنْتَ اللّٰهُ لَا إِلٰهَ إِلَّا أَنْتَ وَحْدَكَ لَا شَرِيْكَ لَكَ ، وَأَنَّ مُحَمَّدًا عَبْدُكَ وَرَسُوْلُكَ.",
    repeatCount: 4,
    translation:
      "O Allah, I have entered the evening, and I call upon You, the bearers of Your Throne, Your angels and all creation, to bear witness that surely You are Allah. There is no god worthy of worship except You Alone. You have no partners, and that Muḥammad ﷺ is Your slave and Your Messenger.",
    virtue:
      "Anas b. Mālik (raḍiy Allāhu ʿanhu) narrates that whoever says it four times in the evening is completely freed from the Hell-fire. Whoever says it in the evening, Allah will forgive the sins he commits in that night. (Abū Dāwūd 5069, 5078)",
  },
  {
    id: "upon-entering-evening",
    title: "Upon Entering the Evening",
    arabic:
      "اَللّٰهُمَّ بِكَ أَمْسَيْنَا وَبِكَ أَصْبَحْنَا وَبِكَ نَحْيَا وَبِكَ نَمُوْتُ وَإِلَيْكَ الْمَصِيْرُ.",
    repeatCount: 1,
    translation:
      "O Allah, by You we have entered the evening and by You we enter upon the morning. By You, we live and we die, and to You is the return.",
    virtue:
      "Abū Hurayrah (raḍiy Allāhu ʿanhu) narrated that the Messenger of Allah ﷺ used to say [the above] when he would enter the evening. (Al-Adab al-Mufrad 1199)",
  },
  {
    id: "good-health",
    title: "Ask Allah For Good Health and Protection",
    arabic:
      "اَللّٰهُمَّ عَافِنِيْ فِيْ بَدَنِيْ ، اَللّٰهُمَّ عَافِنِيْ فِيْ سَمْعِيْ ، اَللّٰهُمَّ عَافِنِيْ فِيْ بَصَرِيْ ، لَا إِلٰهَ إِلَّا أَنْتَ ، اَللّٰهُمَّ إِنِّيْ أَعُوْذُ بِكَ مِنَ الْكُفْرِ وَالْفَقْرِ، وأَعُوْذُ بِكَ مِنْ عَذَابِ الْقَبْرِ، لَا إِلٰهَ إِلَّا أَنْتَ.",
    repeatCount: 3,
    translation:
      "O Allah, grant me well-being in my body. O Allah, grant me well-being in my hearing. O Allah, grant me well-being in my sight. There is no god worthy of worship except You. O Allah, I seek Your protection from disbelief and poverty and I seek Your protection from the punishment of the grave. There is no god worthy of worship except You.",
    virtue:
      "'Abd al-Raḥmān b. Abī Bakrah (raḍiy Allāhu ʿanhu) narrates that his father repeated these words three times in the morning and three times in the evening, following the practice of the Messenger of Allah ﷺ. (Abū Dāwūd 5090, Aḥmad 20430)",
  },
  {
    id: "hasbiyallah",
    title: "Allah Will Suffice You in Everything",
    arabic: "حَسْبِيَ اللّٰهُ لَا إِلٰهَ إِلَّا هُوَ ، عَلَيْهِ تَوَكَّلْتُ ، وَهُوَ رَبُّ الْعَرْشِ الْعَظِيْمِ.",
    repeatCount: 7,
    translation:
      "Allah is sufficient for me. There is no god worthy of worship except Him. I have placed my trust in Him only and He is the Lord of the Magnificent Throne.",
    virtue:
      "Abū al-Dardā’ (raḍiy Allāhu ʿanhu) narrates that whoever recites [the above] seven times in the morning and in the evening, Allah will suffice him in everything. (Ibn al-Sunnī 71)",
  },
  {
    id: "pleased-with-islam",
    title: "Have the Prophet ﷺ Hold Your Hand and Admit You to Paradise",
    arabic: "رَضِيْتُ بِاللّٰهِ رَبًّا ، وَبِالْإِسْلَامِ دِيْنًا ، وَبِمُحَمَّدٍ نَّبِيًّا.",
    repeatCount: 3,
    translation:
      "I am pleased with Allah as my Lord, with Islām as my religion and with Muḥammad ﷺ as my Prophet.",
    virtue:
      "The Messenger of Allah ﷺ said: \"Anyone who says [the above] three times every morning and evening will be pleased on the Day of Judgement.\" (Aḥmad 18927)",
  },
  {
    id: "protect-from-harm",
    title: "Protect Yourself From All Harm",
    arabic:
      "بِسْمِ اللّٰهِ الَّذِيْ لَا يَضُرُّ مَعَ اسْمِهِ شَيْءٌ فِي الْأَرْضِ وَلَا فِي السَّمَاءِ ، وَهُوَ السَّمِيْعُ الْعَلِيْمُ.",
    repeatCount: 3,
    translation:
      "In the Name of Allah, with whose Name nothing can harm in the earth nor in the sky. He is The All-Hearing and All-Knowing.",
    virtue:
      "Whoever says [the above] three times in the evening will not be afflicted by a sudden calamity till the morning. (Abū Dāwūd 5088)",
  },
  {
    id: "sins-forgiven",
    title: "Get Your Sins Forgiven",
    arabic: "سُبْحَانَ اللّٰهِ وَبِحَمْدِهِ.",
    repeatCount: 100,
    translation: "Allah is free from imperfection, and all praise is due to Him.",
    virtue:
      "Whoever recites [the above] in the morning and in the evening 100 times will not be surpassed on the Day of Judgement except by one who says more. (Muslim 2692)",
  },
  {
    id: "unparalleled-reward",
    title: "An Unparalleled Reward",
    arabic:
      "لَا إِلٰهَ إِلَّا اللّٰهُ وَحْدَهُ لَا شَرِيْكَ لَهُ ، لَهُ الْمُلْكُ وَلَهُ الْحَمْدُ ، وَهُوَ عَلَىٰ كُلِّ شَيْءٍ قَدِيْرٌ.",
    repeatCount: 100,
    translation:
      "There is no god worthy of worship except Allah. He is Alone and He has no partner whatsoever. To Him Alone belong all sovereignty and all praise. He is over all things All-Powerful.",
    virtue:
      "Whoever says [the above] 100 times in the morning and evening, none will surpass him in good deeds except one who recited it more. (Nasā'ī in Sunan al-Kubrā 10410)",
  },
  {
    id: "tasbih-tahmid-takbir",
    title: "Tasbih, Tahmid and Takbir",
    arabic: "سُبْحَانَ اللّٰهِ ، اَلْحَمْدُ لِلّٰهِ ، اَللّٰهُ أَكْبَرُ.",
    repeatCount: 33,
    translation: "Allah is free from imperfection. All praise be to Allah. Allah is the Greatest.",
    virtue:
      "The Messenger of Allah ﷺ said reciting Subḥānallāh, Alḥamdulillāh, and Allāhu Akbar before sunset carries immense reward. (Nasā'ī: al-Sunan al-Kubrā 10657)",
  },
  {
    id: "salawat",
    title: "Receive the Intercession of the Prophet ﷺ",
    arabic:
      "اَللّٰهُمَّ صَلِّ عَلَىٰ مُحَمَّدٍ وَّعَلَىٰ اٰلِ مُحَمَّدٍ ، كَمَا صَلَّيْتَ عَلَىٰ إِبْرَاهِيْمَ وَعَلَىٰ اٰلِ إِبْرَاهِيْمَ ، إِنَّكَ حَمِيْدٌ مَّجِيْدٌ ، اَللّٰهُمَّ بَارِكْ عَلَىٰ مُحَمَّدٍ وَّعَلَىٰ اٰلِ مُحَمَّدٍ ، كَمَا بَارَكْتَ عَلَىٰ إِبْرَاهِيْمَ وَعَلَىٰ اٰلِ إِبْرَاهِيْمَ ، إِنَّكَ حَمِيْدٌ مَّجِيْدٌ.",
    repeatCount: 10,
    translation:
      "O Allah, honour and have mercy upon Muhammad and the family of Muhammad as You have honoured and had mercy upon Ibrāhīm and the family of Ibrāhīm. Indeed, You are the Most Praiseworthy, the Most Glorious. O Allah, bless Muhammad and the family of Muhammad as You have blessed Ibrāhīm and the family of Ibrāhīm. Indeed, You are the Most Praiseworthy, the Most Glorious.",
    virtue:
      "Whoever sends ṣalāh upon the Prophet ﷺ 10 times in the morning and 10 times in the evening will receive his intercession. (Ṭabarānī: al-Muʿjam al-Kabīr 6357)",
  },
  {
    id: "protection-from-evil",
    title: "Protection from all Evil",
    arabic: "أَعُوْذُ بِكَلِمَاتِ اللّٰهِ التَّامَّاتِ مِنْ شَرِّ مَا خَلَقَ.",
    repeatCount: 3,
    translation: "I seek protection in Allah's perfect words from the evil of whatever He has created.",
    virtue:
      "Abū Hurayrah (raḍiy Allāhu ʿanhu) narrates that whoever recites [the above] three times in the evening will not be harmed by insect stings in that night. (Tirmidhī 3604)",
  },
];
